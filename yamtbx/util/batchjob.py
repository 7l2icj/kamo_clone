...
class PBS(JobManager):
    ...
    def submit(self, j):
        ...
        if j.nproc > 1:
            cmd = "qsub -j oe -N %s -l ncpus=%d %s" % (self.pe_name, j.nproc, script_name)
        else:
...
class SLURM(JobManager):
    ...
    def submit(self, j):
        ...
        if j.nproc > 1:
            cmd = "sbatch --job-name=%s --cpus-per-task=%d %s" % (self.pe_name, j.nproc, script_name)
        else:
        ...
        if p.returncode != 0:
            raise SgeError("sbatch failed. returncode is %d.\nstdout:\n%s\n"%(p.returncode,
                                                                            stdout))

        r = re.search(r"Submitted batch job ([0-9]+)", stdout[0])
        job_id = r.group(1)
        if job_id == "":
            raise SgeError("cannot read job-id from sbatch result. please contact author. stdout is:\n" % stdout)
    ...
    def qstat(self, job_id):
        ...
        if p.returncode != 0 or len(stdout) == 0:
            print "job %s finished (squeue returned %s)." % (job_id, p.returncode)
            return None
        #raise SgeError("qstat failed. returncode is %d.\nstdout:\n%s\n"%(p.returncode,
        #                                                                     stdout))
