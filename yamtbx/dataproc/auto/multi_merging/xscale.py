...
class XscaleCycles:
    def __init__(self, workdir, anomalous_flag, d_min, d_max,
        ...
        if batch_params.engine == "auto": batch_params.engine = batchjob.detect_engine()
        if batch_params.engine == "sge": self.batchjobs = batchjob.SGE(pe_name=batch_params.sge_pe_name)
        elif batch_params.engine == "pbs": self.batchjobs = batchjob.PBS(pe_name=batch_params.sge_pe_name)
        elif batch_params.engine == "slurm": self.batchjobs = batchjob.SLURM(pe_name=batch_params.sge_pe_name)
